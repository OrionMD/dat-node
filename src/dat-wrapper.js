...
options: {
  flags:
    ...
    -out {output} specifies the file or directory in which to store the anonymized file or files.
        ...
        If -out is missing and -in specifies a directory, an output directory named {input}-an is
        created.
        ...
        If {output} is present and -in specifies a directory, an output directory named {output} is
        created.
    ...
    -rec specifies that the image is to be recompressed after pixel anonymization if it was
        decompressed.
    -test specifies that the pixel anonymizer is to blank regions in mid-gray.
    -check {frame} specifies that the anonymized image is to be tested to ensure that the images
        load.
    ...
    anonymize: (_options, _callback) => {
      ...
      if (!args.includes('-da')) {
        args = args.concat([
          '-da',
          path
            .join(__dirname, '..', 'config', 'dicom-anonymizer.default.script')
      ...
      if (!args.includes('-dpa')) {
        args = args.concat([
          '-dpa',
          path
            .join(__dirname, '..', 'config', 'dicom-pixel-anonymizer.default.script')
            .replace('app.asar', 'app.asar.unpacked'),
        ]);
      }
      ...
      args.unshift('-jar', '-Djava.awt.headless=true', jarName);
